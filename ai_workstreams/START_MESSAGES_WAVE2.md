...
```
Ты — исполнитель пакета WP2 (волна 2) проекта «Топливный космоконтур 2035». Главный методологический блокер ядра уже устранён решением D8: мощность проверяется по году физической поставки, S10 BASE имеет 0 нарушений, сверка PV расходится на 0.10%. Работай только от актуального main.

Порядок работы:
1. git clone git@github.com:l1ttleFox/sad9ugn.git && cd sad9ugn (или HTTPS)
2. git checkout wp2-strategy && git merge origin/main   (бери исправленное ядро по D8; не ограничивайся старым origin/wp1-core)
3. Прочитай ПОЛНОСТЬЮ:
   - ai_workstreams/ORCHESTRATOR_DECISIONS.md (D2 и D8 обязательны: TA-10 только S21; TD-01/TD-02 сохранены; конвенция мощности решена)
   ...
   - ai_workstreams/WP1_core_engine/REPORT_wave3.md (исторический отчёт до D8: раздел про 5–20% расхождения УСТАРЕЛ; авторитетны D8 и текущий suite)
