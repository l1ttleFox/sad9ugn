...
- **Источник метода:** inventory control как ядро space logistics — Ho (2024); запасы как механизм, «мостирующий» lead time, capacity reservation как аналог виртуального запаса — Guo et al. (2025); физическая мотивация отдельного учёта криогенных потерь (boil-off) — Simonini et al. (2024); демонстратор ZBO-класса — RRM3 (по конспекту оркестратора в промпте волны 1).
...
- **Источник метода:** robust/сценарный анализ диверсификации и стоимости гибкого второго источника — Han et al. (2023); раздельное моделирование redundancy / inventory / reservation — Guo et al. (2025); инвестиционные гейты и опционы, закон необходимого разнообразия — Tanaka (2014) по конспекту оркестратора (см. оговорку в `03_methods_evidence_map.md`).
...
- **Результаты:** `[[WP2:final_strategy]]`, сводная таблица `[[WP2:screening_table_25x2]]` — волна 2.
- **Источник метода:** MCDA/adaptive management — Linkov et al. (2006); инвестиционные опционы и гейты — Tanaka (2014, по конспекту оркестратора).
